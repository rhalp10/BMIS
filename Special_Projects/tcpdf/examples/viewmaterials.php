...
function getform($x)
{
	$query = "SELECT format_name FROM formats where format_id  =  $x";	
	$result = @mysql_query ($query); // Run the query.
	$row = mysql_fetch_array($result, MYSQL_NUM);
	$fo ='';
	$fo= $row[0];
...
if(isset($_POST['c_stat'])&& isset($_POST['stat']) )
{
	...
	{
	$st = "1";
	$criteria.="All status - materials that are in and out.<br>";
	}
	else if ($stat=='in')
	{$st = " status='in' ";
	$criteria.="Status - In<br>";
	}
	else if ($stat=='out')
	{$st = " status='out' ";
	$criteria.="Status - Out<br>";
	}
}
if(isset($_POST['c_callno'])&& isset($_POST['callno']) )
{
	$callno = $_POST['callno'];
	if ($callno=='')
	{
	...
	else if (!empty($callno))
	{
	$cn = " and callno like '%$callno%' ";
	$criteria.="Call Number - with '".$callno."'<br>";
	}
}
if(isset($_POST['c_yr'])&& isset($_POST['pub_year']) )
{
	$puby = $_POST['pub_year'];
	$py = " and pub_year =$puby ";
	$criteria.="Publication Year - '".$puby."'<br>";
	
}
if(isset($_POST['c_form'])&& isset($_POST['loadformat']) )
{
	$f = $_POST['loadformat'];
	$fo = " and format_id =$f ";
	$criteria.="Format - '".getform($f)."'<br>";
...
if(isset($_POST['c_dcov'])&& isset($_POST['date_sta']) && isset($_POST['date_end']) )
{
	...
	$dcov = "and (date_acq between '$ds' and '$de') ";
...
$query = "SELECT * from books where $st $cn $dcov $py $fo order by status,format_id,accno,title";	
...
<br>
<big><b>There are currently '.$num.' materials in the system. ['.date('M-d-Y').']</big><br><br>
...
<table align="center" width ="650" cellspacing="2" style="table-layout:fixed" cellpadding="2" border="1">
	...
	while ($row = mysql_fetch_array($result)) {
		...
		<td align=\"left\">' . stripslashes($row['accno']) . '</td>
		<td align=\"left\">' . stripslashes($row['callno']) . '</td>
...
$pdf->Output('library_materials-'.date('Y-M-d').'.pdf', 'I');
