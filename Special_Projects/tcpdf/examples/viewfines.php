...
if(isset($_POST['c_dcov'])&& isset($_POST['date_sta']) && isset($_POST['date_end']) )
{
	...
	$dcov = "and (f.f_date between '$ds' and '$de') ";
...
$query = "
select sum(f_value) from fines f where $cl $dcov order by trans_id desc";	
$result = @mysql_query ($query) or die("Error: ".mysql_error()); // Run the query.
$row = mysql_fetch_array($result, MYSQL_NUM);
$sum=$row[0];

$query = "
select t.trans_id,b.accno,s.studentnum,f.f_value,f.f_collector,f.f_date from fines f 
inner join transactions t on t.trans_id=f.trans_id 
inner join books b on t.accno = b.accno
inner join students s on s.studentnum = t.studentnum
where $cl $dcov 
order by f.trans_id desc
";	
//echo $query;	
$result = @mysql_query ($query) or die("Error: ".mysql_error()); // Run the query.
...
<br>
<big><b>There are currently '.$num.' fine collections in the system. ['.date('M-d-Y').']</big><br><br>
Total fine collected: '.$sum.'<br>
...
<table align="center" width ="650" cellspacing="2" style="table-layout:fixed" cellpadding="2" border="1">
	...
	<td width="80" align="center"><b>Fine Collected</b></td>
	<td width="150" align="center"><b>Received by</b></td>
	<td width="80" align="center"><b>Date Collected</b></td>
